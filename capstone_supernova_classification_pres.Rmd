...
How can we best what are the most impactful variables when it comes to classiyfing supernovas with the given data using a decision/classification tree.
...
get_lightcurve_summary <- function(lightcurve_data_param, potential_supernovas_param){
  ...
  luminosity = calculate_luminosity(flux, true_distmod)) %>% group_by(object_id) %>% mutate(mjd_max_luminosity =  mut_meta_lightcurve_merge_abs_mag[match(max(luminosity), mut_meta_lightcurve_merge_abs_mag$luminosity), ]$mjd, mjd_10pct_luminosity = mut_meta_lightcurve_merge_abs_mag[which(mut_meta_lightcurve_merge_abs_mag$luminosity <= 0.1 * max(luminosity) & mjd > mjd_max_luminosity)[1], ]$mjd)
...
classification_model = rpart(class_label ~ max_absolute_magnitude + min_absolute_magnitude + days_to_10pct + max_luminosity + min_luminosity + mjd_range, data = train_summary, method = 'class')
